...
namespace com.guinealion.animatedBook
{
    ...
    public class LightweightBookHelper : MonoBehaviour
    {
        ...
        /// Opens the book
        /// </summary>
        public void Open(float duration = 0.5f, float openAmount = 1.0f)
        {
            if (openAnimation != null)
                StopCoroutine(openAnimation);
            openAnimation = StartCoroutine(DoChangeOpen(openAmount, duration));
        }
        ...
        IEnumerator DoChangeOpen(float direction, float duration)
        {
            do
            {
